...
import { useDonate } from '@/features/donate/hooks/useDonate'
import { getSimulatedWalletBalance } from '@/features/donate/hooks/usePoolStats'
import { MIN_DONATION, ETHERSCAN_URL } from '@/features/donate/config'
...
function SuccessScreen({ txHash, amount, onDonateAgain, onGoBack }: SuccessScreenProps) {
  const isSimulated = txHash.startsWith('0x_simulated_')
  return (
    <View className="flex-1 bg-gray-50 justify-center px-6">
      <View className="bg-white rounded-3xl p-8 items-center shadow-sm">
        ...
        {isSimulated && (
          <View className="bg-orange-50 border border-orange-100 rounded-xl px-4 py-2 mb-4 w-full">
            <Text className="text-orange-500 text-xs text-center font-medium">
              ⚠️ Mode Simulasi — transaksi tidak benar-benar dikirim ke blockchain
            </Text>
          </View>
        )}
...
export default function DonateFormScreen() {
  ...
  const [walletBalance, setWalletBalance] = useState(() => getSimulatedWalletBalance())
  ...
  const handleReset = useCallback(() => {
    setAmount('')
    setMessage('')
    setWalletBalance(getSimulatedWalletBalance())
    reset()
  }, [reset])
    ...
    >
      ...
      >
        <View className="flex-1 px-5 py-6">
          ...
          <View className="bg-green-50 border border-green-100 rounded-2xl p-4 mb-4 flex-row items-center gap-3">
            <Text className="text-2xl">💰</Text>
            <View className="flex-1">
              <Text className="text-xs text-green-600 font-medium">Saldo Wallet</Text>
